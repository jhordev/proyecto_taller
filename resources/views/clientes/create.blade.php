...
<div class="flex items-center gap-3 mb-6">
    ...
    <div>
        <h1 class="text-xl font-bold text-slate-900">Nuevo Cliente</h1>
        <p class="text-slate-500 text-sm">Registro de información personal.</p>
    </div>
</div>

<form action="{{ route('clientes.store') }}" method="POST" class="space-y-6 max-w-6xl">
    @csrf
    ...
    <div class="bg-white border border-slate-200">
        ...
        <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-5">
            <x-field label="Nombres" name="nombre" placeholder="Nombres del cliente" />
            <x-field label="Apellidos" name="apellido" placeholder="Apellidos del cliente" />
        </div>
    </div>
        ...
        <div class="p-5 grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-5">
            <div>
                ...
                <select name="tipo_doc" class="w-full px-3 py-2 border border-slate-300 text-sm text-slate-900 focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500" style="border-radius:0">
                    ...
                    <option value="CARNET DE EXTRANJERÍA" {{ old('tipo_doc') == 'CARNET DE EXTRANJERÍA' ? 'selected' : '' }}>CARNET DE EXTRANJERÍA</option>
                </select>
            </div>
            <x-field label="Número de Documento" name="nro_doc" placeholder="Ej: 12345678" />
        </div>
    </div>
    ...
    <div class="bg-white border border-slate-200">
        ...
        <div class="p-5 grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-5">
            ...
            <x-field label="Email" name="email" type="email" :required="false" />
        </div>
    </div>
